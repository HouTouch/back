...
exports.editProduct = (req, res) => {
  ...
  const product_all_price = product_in_warehouse_number * product_single_price;
  const sql = 'update product set product_name = ?,product_category = ?,product_unit = ?,product_in_warehouse_number = ?,product_single_price = ?,product_all_price = ? ,product_update_time= ?,in_memo = ? where id = ?';
  db.query(
    sql,
    [
      product_name,
...
// 再次申请出库
exports.applyOutProductAgin = (req, res) => {
  const product_out_status = '申请出库'
  const {
    id,
    product_out_id,
...
exports.audioProduct = (req, res) => {
  ...
  if (product_out_status == "同意") {
    ...
    db.query(sql, {
      ...
      if (err) return res.cc(err)
      const sql1 =
        'update product set product_in_warehouse_number = ?,product_all_price = ?,product_out_status = NULL ,product_out_id = ?,product_out_number =NULL,product_out_apply_person=NULL,apply_memo =NULL,product_out_price =NULL,product_apply_time = NULL where id = ?'
      db.query(sql1, [newWarehouseNumber, product_all_price, product_out_id, req.body.id], (err, result) => {
        if (err) return res.cc(err)
        res.send({
          status: 0,
          message: '产品出库成功'
...
// 监听换页返回数据  产品页面
// limit 5 为我们要拿到数据 offset 我们跳过多少条数据
exports.returnProductListData = (req, res) => {
    const number = (req.body.pager - 1) * 5
    const sql =
        `select * from product where product_in_warehouse_number>= 0 ORDER BY product_create_time limit 5 offset ${number} `
    db.query(sql, (err, result) => {
        if (err) return res.cc(err)
        res.send(result)
    })
...
exports.returnApplyProductListData = (req, res) => {
    const number = (req.body.pager - 1) * 5
    const sql =
        `select * from product where product_out_status = "申请出库" || product_out_status = "否决" ORDER BY product_apply_time limit 5 offset ${number} `
    db.query(sql, (err, result) => {
        if (err) return res.cc(err)
        res.send(result)
    })
...
exports.returnOutProductListData = (req, res) => {
    const number = (req.body.pager - 1) * 5
    const sql = `select * from outproduct ORDER BY product_audit_time limit 5 offset ${number} `
    db.query(sql, (err, result) => {
        if (err) return res.cc(err)
        res.send(result)
    })
