...
import {searchProductForOutId, returnOutProductListData, auditProductList} from '@/api/product'
import {
    Search
...
const getOutProductListLength = async () => {
    const res = await auditProductList() as any
...
const getFirstPageList = async () => {
    const res = await returnOutProductListData(1) as any
...
const currentChange = async (value: number) => {
    ...
    const res = await returnOutProductListData(paginationData.currentPage) as any
    tableData.value = res.data
}
...
const searchProductOutId = async () => {
    const res = await searchProductForOutId(productOutId.value as number) as any
    tableData.value = res.data
}
</script>
