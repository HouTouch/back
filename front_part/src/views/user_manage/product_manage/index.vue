...
const searcheAdmin = async () => {
   ...
   if (res.data.result.length != 0) {
      ...
      const res = await returnListData(0, '产品管理员')
      tableData.value = res.data.result
   }
}
// 创建管理员
...
<template>
   ...
   <div class="table-wrapped">
      ...
      <div class="table-footer">
         <el-pagination :page-size="2" :current-page="paginationData.currentPage" :pager-count="7" :total="adminTotal"
            :page-count="paginationData.pagecount" layout="prev, pager, next" @current-change="currentPage" />
      </div>
   </div>
   <create ref="Create" ></create>
   <editAdmin ref="Edit" ></editAdmin>
   <deleteAdmin ref="DeleteRef" ></deleteAdmin>
</template>
