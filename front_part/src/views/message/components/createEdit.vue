...
const departmentReceiptValue = ref()
...
const confirmFnc = async () => {
    if (title.value == '发布公告') {
        formData.message_content = editorRef.value.getHtml().slice(3, -4)
    ...
    <el-dialog v-model="dialogFormVisible" :title='title' width="50%" align-center append-to-body
            ...
            <el-form :model="formData" label-width="120px" style="max-width: 600px;" :rules="rules"
                ...
                <el-form-item label="发布部门" prop='message_publish_department' v-if="title == '发布公告' || title == '编辑公告'">
                    <el-select v-model="formData.message_publish_department" placeholder="请选择发布部门">
                        <el-option v-model="departmentValue" v-for="(item,index) in departmentList" :key="index" 
                ...
                <el-form-item label="接收部门" prop='message_receipt_object' v-if="title == '发布公告' || title == '编辑公告'">
                    <el-select v-model="formData.message_receipt_object" placeholder="请选择接收部门">
                        <el-option v-model="departmentReceiptValue" v-for="(item, index) in departmentList" :key="index"
                            :label="item" :value="item"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="内容" prop='message_content'>
                    <div style="border: 1px solid #ccc">
                        <!-- wangEditor结构 -->
                        <Toolbar style="border-bottom: 1px solid #ccc;width: 400px;" :editor="editorRef"
                            :defaultConfig="toolbarConfig" :mode="mode" />
                        <Editor style="height: 300px; overflow-y: hidden;width: 400px;" v-model="valueHtml"
        ...
        <template #footer>
            <span class="dialog-footer">
                <el-button type="primary" @click="confirmFnc">
                    确定
                </el-button>
            </span>
        </template>
