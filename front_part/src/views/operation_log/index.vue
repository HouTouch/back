...
const getOperationListLength = async () => {
  ...
  paginationData.operationTotal = res.data.length;
  paginationData.operationPageCount = Math.ceil(res.data.length / 10);
};
...
const operationCurrentChange = async (value: number) => {
  ...
  const res = await returnOperationListData(
...
const searchOperationPerson = async () => {
  const res = await searchOperationLogList(name.value);
  tableData.value = res.data;
};
